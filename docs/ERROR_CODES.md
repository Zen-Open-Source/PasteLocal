...
All clipbridge errors follow the format `{"ok": false, "code": "CBxxxx",
...
| CB1002 | 500 Internal Server Error | Clipboard tool not installed | Run `clipbridge doctor --fix` |
| CB1003 | 500 Internal Server Error | Clipboard tool failed | Check `clipbridge logs` |
...
| CB2001 | 401 Unauthorized | Invalid auth token | Re-run `clipbridge add-host <host>` to sync the token. |
...
The `clipbridge-remote` binary maps server error codes to exit codes:
